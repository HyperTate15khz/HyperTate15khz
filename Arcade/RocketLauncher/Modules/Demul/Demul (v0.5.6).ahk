...
lastControlUsedNaomi := moduleIni.Read("Settings", "LastControlUsedNaomi", "standard","",1)
lastControlUsedAtomiswave := moduleIni.Read("Settings", "LastControlUsedAtomiswave", "standard","",1)
...
If (ident = "dc")
{
	...
	If (LastControlUsed != controls) {
		If (ident = "atomiswave")
		{
			...
			moduleIni.Write(controls, "Settings", "LastControlUsedAtomiswave")	; write control loaded to the ini so we know what we used last for next launch
		} Else {
			...
			moduleIni.Write(controls, "Settings", "LastControlUsedNaomi")	; write control loaded to the ini so we know what we used last for next launch
		}
	}
}
